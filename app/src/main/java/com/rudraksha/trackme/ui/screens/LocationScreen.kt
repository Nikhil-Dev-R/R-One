...
import com.rudraksha.trackme.ui.components.RCard
import com.rudraksha.trackme.viewmodel.FitnessData
...
//        mapStyleOptions = MapStyleOptions.loadRawResourceStyle(context, com.google.android.material.R.raw.style),
    ...
    var showOnMapClickMarker by remember { mutableStateOf(false) }
...
fun checkPermission(context: Context, permissionList: List<PermissionState>): Boolean {
    for (permission in permissionList) {
        if (!permission.status.isGranted) {
            Log.d("checkPermission", "Permission not granted")
            if (permission.permission == android.Manifest.permission.RECORD_AUDIO ||
                permission.permission == android.Manifest.permission.ACCESS_BACKGROUND_LOCATION
            ) {
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q) {
                    ...
                    // Request for the permission if current android version is less than 29
                    permission.launchPermissionRequest()
                    if (!permission.status.isGranted) {
                        return false
                    }
